...
test('现货倒卖：现货采购单(按毛重付款·纸筒空差算实际单价)计入毛利', () => {
  ...
  // 采购重量 + 实际重量；金额=实际优先(未填用采购重量)×单价
  if (!/var qty=act>0\?act:buy;/.test(html)) throw new Error('应实际重量优先(未填用采购重量)');
  if (!/var amt=qty\*\(base\+extra\);/.test(html)) throw new Error('金额应=数量×(基础单价+加价)');
