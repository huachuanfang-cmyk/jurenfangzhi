...
test('送货单多色多缸出货时按颜色和缸号分组打印', () => {
  const fn = extractFunction('printFGOut');
  if (!fn) throw new Error('printFGOut not found in index.html');

  const checks = [
    { field: '出货分组', pattern: /fgOutGroups|outGroups/ },
    { field: '多色标签', pattern: /多色/ },
    { field: '多缸标签', pattern: /多缸/ },
    { field: '组内小计', pattern: /groupKG|grpKG|小计/ },
    { field: '分组行样式', pattern: /grp-row|group-row/ },
  ];

  const failures = checks.filter(c => !c.pattern.test(fn));
  if (failures.length) {
    throw new Error('多色出货打印缺少: ' + failures.map(f => f.field).join(', '));
  }
});
